...
description: |
  ...
  baoyu-post-to-wechat（发布），在图片生成处自然暂停等待用户，其余全自动。
...
将「有原始素材」到「草稿箱」之间所有重复步骤自动化，只保留一个必须人工介入的节点：**图片生成**（目前暂无图片生成 API，需手动在 Gemini 等工具操作）。
...
- **默认全自动**：Step 0 → Step 2 连续运行，不中途停下。
- **唯一暂停点**：Step 3（等待用户手动生成图片）。
...
```bash
COVER_SKILL=$(find ~/.claude/plugins/cache/baoyu-skills -name "SKILL.md" \
...
工作流：
  1. 打开 https://gemini.google.com（或其他图片生成工具）
  2. 把提示词内容粘贴进去生成图片
  3. 下载后放到上面对应的路径
...
| `cover.jpeg` 不存在，用户确认跳过 | 继续，发布时用默认封面（微信会显示默认封面），在 meta.yaml 记录 `no_cover: true` |
| `cover.jpeg` 不存在，用户没确认 | 再次提示，等待 |

**更新 meta.yaml** `status: "images_ready"`，记录 `cover_images: {COVER_EXISTS}`，`body_images: {IMAGE_COUNT}`
...
```bash
FORMAT_SKILL=$(find ~/.claude/plugins/cache/baoyu-skills -name "SKILL.md" \
...
[ -z "$WECHAT_APP_ID" ] || [ -z "$WECHAT_APP_SECRET" ] && {
  ...
  echo "请先运行 /baoyu-post-to-wechat 完成首次配置，再使用 pipeline 发布。"
  exit 1
}
```
...
| `drafted` | Step 2 | 原稿已有，还未生成提示词 |
| `prompts_ready` | Step 3 | 提示词已有，等待用户放图片 |
| `images_ready` | Step 4 | 图片已放好，需要格式化 |
